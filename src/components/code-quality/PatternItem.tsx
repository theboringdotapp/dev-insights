...
function PatternItem({ pattern, index }: PatternItemProps) {
  ...
  const getFrequencyLabel = () => {
    if (pattern.frequency.toLowerCase().includes("very common")) return "trending";
    if (pattern.frequency.toLowerCase().includes("occasional")) return "case specific";
    return "common";
  };
  ...
  return (
    <li className="flex flex-col">
      ...
      >
        ...
        <div className="flex flex-col text-left flex-grow">
          ...
          <div className="flex items-center text-zinc-500 dark:text-zinc-500 mb-1">
            {getCategoryIcon()}
            <span className="ml-1 text-xs">{pattern.category.charAt(0).toUpperCase() + pattern.category.slice(1)}</span>
          </div>
      ...
      {isExpanded && (
        <div 
          className="mt-2 mb-4 rounded text-xs w-full border border-zinc-200 dark:border-zinc-700 opacity-100 transition-opacity duration-200"
        >
          <div className="bg-zinc-100 dark:bg-zinc-800 px-3 py-1 text-xs font-mono border-b border-zinc-200 dark:border-zinc-700 flex items-center justify-between">
            ...
            <span className="text-xs text-zinc-500 whitespace-nowrap flex-shrink-0">Impact & Context</span>
          </div>
          ...
          <div className="p-3 bg-white dark:bg-zinc-900 text-left">
            ...
            <div className="mt-2">
              <h5 className="text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Impact:</h5>
